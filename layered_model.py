...
# Define model
model = define_dense_model_single_layer(input_length=784, activation_f='sigmoid', output_length=1)

# Train model
target_digit = 2
model = fit_mnist_model_single_digit(x_train, y_train, target_digit, model, epochs=10, batch_size=128)

# Evaluate model
loss, accuracy = evaluate_mnist_model_single_digit(x_test, y_test, target_digit, model)
